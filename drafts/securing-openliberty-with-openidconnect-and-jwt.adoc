...
Open Liberty now includes more open source security features to enable you to secure your apps with external security providers. Let's take a quick look at why and how to use some of these features.

== Authenticating microservices on Open Liberty
Open Liberty provides support for writing apps that authenticate users with social media logins using Open ID standards-based technologies. You can present users of your web app with the options to log in using their existing Facebook, GitHub, Google, LinkedIn, or Twitter IDs. You can also write your own integrations with other social media platforms if you need to.  Alternatively, you can enable your app users to use single sign-on by authenticating with a SAML identity provider.

Using Open ID Connect or SAML providers ensures that the secured app never sees the user's password, and relieves developers and administrators of the effort of managing user accounts. In addition, providing a single sign-on experience to users enables them to log in once for all secured apps that they use.

Open Liberty also supports (JWT), which are identity credentials with a standardized format that can be exchanged between processes. They're secured against tampering by use of digital signatures. So your microservices can securely exchange requested information on behalf of users. OpenID Connect providers can issue Json Web Token (JWT) credentials.  These tokens can be submitted to downstream secured microservices to propagate identity in an efficient, self-contained way.  Microservices don't need to contact the identity provider again.
...
+
* A server called `oidc_provider` with the OpenID Connect Provider (`openIdConnectProvider-1.0`) provides user authentication for the OpenID Connect client.
...
The servlet then makes a REST call to the microservice.  REST clients can be configured to automatically propagate a JWT token if they have one, so the JWT gets sent along in the request. The microservice server is configured to trust JWTs issued by the provider, so it grants access to the JAX-RS service.
