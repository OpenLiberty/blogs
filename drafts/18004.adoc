...
As you'll have come to expect from the Open Liberty project, we've got the latest MicroProfile version all done and dusted before the end of the year. We like to think of MicroProfile 2.1 as "laser-focused" with its singular dedication in this release to updating the MicroProfile Open Tracing 1.2 feature. Open Tracing 1.2 gives you enhancements to how you trace the flow of requests between microservices. And, of course, MicroProfile 2.1 comes with a convenience feature so that you can enable all the features in one go, if you want. Beyond that, on the ops side, there are connection pool metrics available from the Liberty runtime which you can now access from the `/metrics` endpoint that is provided by the MicroProfile Metrics feature.
...
*  <<mpOpenTracing,Trace the flow of requests between microservices (MicroProfile OpenTracing 1.2)>>
...
[#mpOpenTracing]
== Trace the flow of requests between microservices (MicroProfile OpenTracing 1.2)
