...
Open Liberty continues with its support of Eclipse MicroProfile by providing the `microProfile-2.1` feature. MicroProfile 2.1 was just announced by the community on October 19. Keeping to its time-boxed release mantra, MicroProfile 2.1 contains an update to a single component this time around: OpenTracing 1.2. And, within two months, Open Liberty has a production-ready, https://www.ibm.com/marketplace/elite-support-for-open-liberty[fully-supported] implementation available.
