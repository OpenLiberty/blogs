...
As you'll have come to expect from the Open Liberty project, we've got the latest MicroProfile version all done and dusted before the end of the year. We like to think of MicroProfile 2.1 as "laser-focused" with its singular dedication in this release to updating just the MicroProfile Open Tracing 1.2 feature (but keep an eye out for the feast of goodies planned for https://github.com/eclipse/microprofile/milestone/1[MicroProfile 2.2] in February).
