...
seo-title: SEO-TITLE:-on Open Liberty 20.0.0.5
seo-description: Open Liberty 20.0.0.5 provides  ........
blog_description: Open Liberty 20.0.0.5 provides ........
---
...
// tag::intro[]
Open Liberty 20.0.0.5 provides . 
...
JAAS Custom Login Modules enables you to use this feature embedded within a resource adapter or application.

The updated JAAS Custom Login Modules now means it is now possible for JAAS Custom Login Modules to remain packaged within a resource adapter. It does not have to repackage JAAS Custom Login Modules into a separate library for Liberty to configure them. Even if you are not using a resource adapter at all, it is now possible to package JAAS Custom Login Modules within your application.
...
You can enable fail over for EJB persistent timers that are configured to persist timers to the same database. This feature adds a new configurable attribute, `missedTaskThreshold`, which specifies the maximum amount of time that you want to allow for an execution of a persistent timer to complete before allowing another server to take over and run it instead. Previously, it was only possible to partly coordinate automatic EJB persistent timers across multiple Liberty servers by configuring the EJB timer service to persist timers to the same database. This caused a single timer instance to be created on one of the servers, but without the ability to fail over to another server if the original server stops or crashes.
