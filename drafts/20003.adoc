...
seo-title: title in Open Liberty 20.0.0.3 - OpenLiberty.io
seo-description: In Open Liberty 20.0.0.3, description.  
blog_description: In Open Liberty 20.0.0.3, description.
---

= Allow the SameSite attribute to be set on a Cookie in Open Liberty 20.0.0.3
...
// tag::intro[]
In Open Liberty 20.0.0.3, you now have the ability to add the SameTime Cookie on the session, having access to three values on the SameTime attribute. As well, you now have the ability to use the Kafka client API allowing users to access Kafka specific message properties, not just the payload of the specific message as you had previously with MicroProfile Reactive `Message` API 

In link:{url-about}[Open Liberty] 20.0.0.3:

* <<ASCA,Adding the SameTime cookie attribute>>
...
//tag::features[]
[#ASCA]
== Adding the SameTime cookie attribute 
...
The Servlet `javax.servlet.http.Cookie` API does not offer the ability to set the SameSite attribute on a Cookie. If the SameSite attribute is needed, the options for setting it are currently limited to using the `HttpServletResponse.addHeader` and `HttpServletResponse.setHeader` and constructing the Set-Cookie header. The SameTime attribute is used by Browsers to determine if a particular cookie should be sent with a request. 
...
To use SameTime cookie attribute:
...
At the end of one or more cookie names. The below snipped would map the following cookie name to SameSite attributes:
