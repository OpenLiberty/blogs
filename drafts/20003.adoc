...
seo-title: In Open Liberty 20.0.0.3, you can now add the SameTime Cookie on the session, with three values on the SameTime attribute. As well you can use the Kafka client API, allowing users to access Kafka specific message properties, not just the payload of the specific message as you had previously with MicroProfile Reactive `Message` API - OpenLiberty.io
