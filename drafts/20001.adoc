...
# OAuth server scenario: causes the public keys from /tmp/trustedcert.pem (written by getcerts.sh) to be added to Liberty trust store.
...
No constraints (32 processors) - the old version and new version display the same data.
