...
Open Liberty 23.0.0.11 includes a security fix and new vendor metrics for MicroProfile Metrics that you can add directly to your dashboards in various monitoring tools, without any additional computation on your part.  Also announced in this release, but available since 23.0.0.10, is the ability to include all files in a specified directory in your server configuration. 
...
The new vendor metrics are available in the `/metrics` output when you enable the following Microprofile Metrics 5.0 feature in your `server.xml`:
