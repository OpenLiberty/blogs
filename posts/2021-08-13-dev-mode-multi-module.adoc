...
== Running Open Liberty on a multi-module project

Open Liberty can be run on a multi-module project with a single command by using dev mode or dev mode with container support.  To use any of these options, you must first define the Liberty Maven plug-in.
