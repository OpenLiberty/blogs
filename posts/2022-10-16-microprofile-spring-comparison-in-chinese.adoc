...
Liberty、Quarkus、Payara、TomEE、Helidon、KumuluzEE 等。您可以在https://wiki.eclipse.org/MicroProfile/Implementation[此处]找到实现细节。
...
请参阅https://openliberty.io/guides/rest-intro.html[此]Open Liberty
以了解有关 JAX-RS 的更多信息。
...
MicroProfile
使用https://github.com/eclipse/microprofile-open-api/releases[MicroProfile
...
更新。开发人员不必因为配置值更改而重新打包微服务。设计原则是这些配置可以存储在微服务外部的某个地方，并且这些配置可供微服务使用。link:https://12factor.net/[这被称为外部化配置，这是12
因素 APP]强调的因素之一。下面我们来看看 MicroProfile 和 Spring
是如何帮助我们配置微服务的。
