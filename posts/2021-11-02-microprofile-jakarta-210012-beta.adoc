...
With this feature, you can use MicroProfile together with Jakarta EE 9.1., whereas in prior releases and versions of MicroProfile Health were for the Java EE platform and does not provide support for Jakarta EE 9.1 applications.
...
You can enable the MicroProfile Metrics 4.0 feature by using the `mMetrics-4.0` feature in the server.xml file,
...
With this property set, deploy several JAX-RS applications and visit `/openapi/ui` on your liberty server where you should see documentation for all the applications.
...
** If set, the info section in the final OpenAPI document is replaced with the value of this config property.
** The value must be a valid OpenAPI Info section in JSON format.
** This replacement is done after any merging has taken place
