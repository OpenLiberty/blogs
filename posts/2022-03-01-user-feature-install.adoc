...
A feature can consist of a single OSGi bundle and a feature manifest file. In this post, we’ll use Maven plugins to build and install the Liberty feature. 
...
We will be using `Eclipse` to create Maven projects. 
For more info, check out link:https://www.ibm.com/docs/en/wasdtfe?topic=projects-creating-maven[Creating Maven projects].

In this blog, you will do the followings :
...
. Build the Bundle
+
To use the maven-bundle-plugin, explicitly specify `packaging` to "bundle" in the `pom.xml`. 
+
[source, xml]
----
...
----
+
We also need to specify the configuration. `Bundle-SymbolicName` is the only required header; Other headers are optional, but it's good practice to include name and version.
+
In the previous step, we created a Java class that implements OSGi `BundleActivator`. To activate the BundleActivator, we need to identify the bundle activator class to the OSGi framework by adding the `Bundle-Activator` header. 
+
[source, xml]
----
<plugin>
...
The Enterprise Subsystem Archive (ESA) is an archive file (i.e. zip) containing the SUBSYSTEM.MF manifest file. The contents of this manifest file provides information on how to install, resolve and start the bundle.

A Liberty feature requires the following headers:
...
- *Subsystem-Type* : All features are currently of the same subsystem type `osgi.subsystem.feature`;
...
----
By default it will not generate a manifest, so we have to set the `generateManifest` header to *true*. Note that in order to install the feature, we need to set the *visibility* directive to "public". We can do so by setting the `Subsystem-SymbolicName` header to "_Bundle-SymbolicName;visibility:=public_". If the *visibility* is set to `protected|private`, Liberty Maven Plugin will not be able to resolve the feature. Also, the mandatory header `Subsystem-Content` will be created automatically by the plugin. 
...
Create a Bill of Materials (BOM) for the user feature ESA file. The BOM is a pom file that manages the dependencies of the project. Liberty Maven Plugin's `prepare-feature` goal and `install-feature` goal require the BOM file to install the Liberty user feature onto Liberty runtime. 
[source, xml]
.pom.xml
----
...
+
Open `${project.build.testOutputDirectory}/wlp/server.xml` file and add the user feature. The extension `usr` indicates that the feature will be installed to `${project.build.testOutputDirectory}/wlp/usr` directory. It also tells the server where to find the user feature when the server starts. 
+
[source, xml]
.server.xml
----
...
- To learn more about OSGi applications, see https://www.ibm.com/docs/en/wasdtfe?topic=developing-osgi-applications[Developing OSGi applications].
