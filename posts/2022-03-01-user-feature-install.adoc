...
You can expand the capability of Liberty by writing your own Liberty features and installing them onto the Liberty server. For example, you can develop a user feature to configure a custom authentication provider or incorporate custom functions that aren't shipped with Liberty. Liberty also provides various System Programming Interfaces (SPIs) that you can use to extend the runtime environment.  
