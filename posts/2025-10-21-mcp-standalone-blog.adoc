...
4. Inside your `pom.xml` file, add the version number from step 3 and the path to your `wlp` folder as variables named `mcp_jar_version` and `wlp-dir-path` within the `<properties>` block:
+
[source,java]
----
<properties>
    <wlp-dir-path>replace-with-path-to-wlp-dir</wlp-dir-path>
    <mcp_jar_version>replace-with-version-number</mcp_jar_version>
</properties>
----
...
<dependency>
    ...
    <version>${mcp_jar_version}</version>
    <scope>system</scope>
    <systemPath>${wlp-dir-path}/lib/io.openliberty.mcp_${mcp_jar_version}.jar</systemPath>
</dependency>
----
