...
== How to Use the MCP Server Feature
...
Note: The `destructiveHint` and `idempotentHint` are only meaningful when `readOnlyHint` is set to false, since these hints contradict one another.
...
Here are the simple steps to follow to try out the MCP Server feature `mcpServer-1.0`` for yourself:

1. Download the Open Liberty All Beta Features package from [here](https://openliberty.io/start/#runtime_betas)
...
4. Inside your `pom.xml` file, add the version number from step 3 and the path to your `wlp` folder as variables named `version` and `wlp-dir-path` within the `<properties>` block:
+
[source,java]
----
<properties>
...
<dependency>
    ...
    <scope>system</scope>
    <systemPath>${wlp-dir-path}/lib/io.openliberty.mcp_${version}.jar</systemPath>
</dependency>
----
