...
You can easily declare a tool with the Liberty MCP Server feature by adding the `@Tool` annotation to the desired method. This method must exist within a https://openliberty.io/docs/latest/cdi-beans.html[CDI managed bean] that has a scope annotation (e.g. `@ApplicationScoped` or `@RequestScoped`).
