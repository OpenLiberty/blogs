...
These convenience features enable you to rapidly develop applications using all of the APIs contained in the Web Profile and Platform specifications. If you've previously used the `webProfile-9.1` Liberty convenience feature, note that, unlike the `webProfile-9.1` feature, the `webProfile-10.0` Liberty feature does not enable the XML Binding feature because XML Binding is not a Web Profile component specification. If your application relies on XML Binding APIs, you must explicitly enable the `xmlBinding-4.0` Liberty feature when you use the `webProfile-10.0` feature.
