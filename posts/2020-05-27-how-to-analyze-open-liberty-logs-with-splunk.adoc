...
. Create the Splunk HTTP Event Collector data input token. In the Splunk Web Interface, go to *Settings -> Data Inputs -> HTTP Event Collector -> New Token*. Enter a name for the HTTP Event Collector, in this example, we named it "liberty", and leave the other fields unchanged. Click *Next*. In the Input Settings, ensure the `Source type` field is set to “Automatic”. Leave the other fields unchanged and click *Review*.
