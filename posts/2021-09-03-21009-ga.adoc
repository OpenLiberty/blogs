...
* link:https://github.com/OpenLiberty/open-liberty/issues/17444[EL selects wrong method if varargs were used with method overloading]
+
An `EL-3.0` performance patch in 21.0.0.3 inadvertently ported over `varargs` matching code, which later identified a bug in the code. The EL implementation would select the wrong method if `varargs` were used when overloading a method. The code has since been updated upstream in the Tomcat community (see Bugzilla Issue 65358), and it has also been ported over to Liberty. 
