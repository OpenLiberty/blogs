...
seo-description: Open Liberty 23.0.0.4-beta provides an update to the Jakarta Data preview, introducing a way to combine multiple ways of specifying ordering and sorting.
blog_description: Open Liberty 23.0.0.4-beta provides an update to the Jakarta Data preview, introducing a way to combine multiple ways of specifying ordering and sorting.
...
Open Liberty 23.0.0.4-beta provides an update to the Jakarta Data preview, introducing a way to combine multiple ways of specifying ordering and sorting.
...
This Jakarta Data beta update allows you to combine multiple ways of specifying ordering and sorting, defining a precedence.  Sorting defined by `OrderBy` (annotation or query-by-method keyword) is applied first, followed by `Sort` parameters (on the method or `Pageable`).

Building on from the code snippet provided in the original preview, the following is an example of using both `OrderBy` and `Sort`:
