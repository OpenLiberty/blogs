---
layout: post
title: "Calling LLM tools with LangChain4j in MicroProfile and Jakarta EE applications"
# Do NOT change the categories section
categories: blog
author_picture: https://avatars3.githubusercontent.com/AndrewSasmito
author_github: https://github.com/AndrewSasmito
seo-title: Calling LLM tools with LangChain4j in MicroProfile and Jakarta EE applications
seo-description: Learn how to call LLM tools using the LangChain4j library in Jakarta EE and MicroProfile applications  running on Liberty written in Java.
blog_description: "Learn how to call LLM tools using the LangChain4j library in Jakarta EE and MicroProfile applications."
...
. Set the GitHub key environment variable: `export GITHUB_API_KEY=<your GitHub API token>` or `set GITHUB_API_KEY=<your GitHub API token>`
...
. Set the Ollama base url environment variable: `export OLLAMA_BASE_URL=http://localhost:11434` or `set OLLAMA_BASE_URL=http://localhost:11434`
...
. Create a new key
. Set the Mistral API key environment variable: `export MISTRAL_AI_API_KEY=<your Mistral AI API key>` or `set MISTRAL_AI_API_KEY=<your Mistral AI API key>`
...
* **Front end** (`toolChat.js` and `toolChat.html`) - Provides the user interface
...
5. LangChain4j runs the corresponding Java method with those parameters.
...
This implementation uses the https://api.stackexchange.com/[Stack Overflow API] to enhance the LLM's capabilities. When the user asks a technical question that might benefit from current Stack Overflow data, the LLM calls the appropriate search method:

1. The search queries the Stack Exchange API to find relevant questions.
2. The search retrieves the top answer for each question.
3. The search returns the formatted results to the LLM.
4. The LLM then synthesizes the returned information into a coherent response.
...
* `@Tool` annotation provides a description that helps the LLM understand when the tool is appropriate to use, what functionality it provides, and what the expected output format is.
* `@P` annotation describes each parameter so the LLM can understand what information to provide, format the parameter correctly, and extract relevant details from the user's query.
...
* Each WebSocket connection gets a unique `sessionId` that's passed to the AI service to maintain conversation context. 
...
* Using `@Tool` and `@P` annotations to expose Java methods to LLMs
