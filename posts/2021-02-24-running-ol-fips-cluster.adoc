...
**Summary:** Security is top priority for any organization. Whether FIPS is mandatory or optional for your organization, different level of FIPS can be enabled for Open Liberty application using appropriate level of JDK. It is important to take into the consideration all the dependencies of the application before enabling FIPS to make sure that application will work. If you are already using Liberty applications with FIPS enabled on-premises, it is easy to move to Kubernetes by making sure that you pick the Liberty image with IBM JDK 8 and update the container image with FIPS enabled files.
