---
layout: post
title: "Disable LTPA cookies for TAI/SPNEGO or JWT cookies for JWT SSO on Open Liberty 20.0.0.7"
categories: blog
author_picture: https://avatars3.githubusercontent.com/tomjenningss
author_github: https://github.com/tomjenningss
seo-title: Disable LTPA cookies for TAI/SPNEGO or JWT cookies for JWT SSO on Open Liberty 20.0.0.7 - OpenLiberty.io
...
---
= Disable LTPA cookies for TAI/SPNEGO or JWT cookies for JWT SSO on Open Liberty 20.0.0.7
...
With Open Liberty 20.0.0.7 users have the availability to configure the return LTPA cookie for TAI/SPNEGO or JWT SSO cookie configurations.

In link:{url-about}[Open Liberty] 20.0.0.7:

* <<LTPA-cookie, Choose to disable LTPA cookie for TAI/SPNEGO>>
* <<JWT-cookie, Choose to disable JWT SSO cookie>>
...
We now make the Kernel package available for download along with our other existing packages inclduing: Jakarta EE 8, Web Profile 8 and MicroProfile 3. Use the `featureUtility` to add only the features that you need to the kernel, helping to minimize the runtime's footprint. Take a look at our link:{url-prefix}/downloads/[Downloads page].
...
LTPA cookie encapsulates an encrypted authentication token containing the user identity and expiration information and can be used for single sign-on (SSO). Users now have the option to decide to receive the LTPA cookie for TAI and SPNEGO authentication.
