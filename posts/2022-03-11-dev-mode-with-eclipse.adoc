...
Liberty dev mode enables developers to quickly and iteratively develop cloud-native Java applications with the latest Jakarta EE and MicroProfile technologies. Dev mode will watch your project for file changes and provide hot reload and deployment, on demand testing, and debugger support. You can work with dev mode from anywhere, whether it's from a terminal with a text editor or your favorite IDE (integrated development environment). This blog post details the current best practice for using dev mode to develop your application within the Eclipse IDE.

Check out a demo of working with Liberty dev mode in the Eclipse IDE:
++++
<iframe width="560" height="315" src="https://youtu.be/613VBYdk6f8" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
++++
...
Start dev mode through the Eclipse IDE terminal ("Window" -> "Show View" -> "Terminal") or through your operating system's terminal with the following commands:
...
You can attach a debugger to the running server to step through your code at any time. In Eclipse you can create a debug configuration by "Run" -> "Debug Configurations..." -> "Remove Java application". The default port for debugging is `7777`.
...
If you have https://marketplace.eclipse.org/content/ibm-liberty-developer-tools[Liberty Developer Tools] installed in your Eclipse workspace, the server configuration editor views can be helpful as you modify your `server.xml` and other configuration files.
...
* For Maven projects: Window -> Preferences -> Maven -> Liberty Integration to "No".

* For Gradle projects: Window -> Preferences -> Gradle -> Liberty Integration to "No".
