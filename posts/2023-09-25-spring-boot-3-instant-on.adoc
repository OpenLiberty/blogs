...
== Summary
Open Liberty InstantOn provides a holistic runtime for deploying cloud-native applications that have rapid startup. Liberty InstantOn can be applied to applications using standards, such as Jakarta EE and MicroProfile, as well as Spring based applications using the latest versions of Spring Boot and Spring Framework that have support for `org.crac`. Applications will continue to benefit from the other advantages the Open Liberty runtime provides, such as:
...
Using the production ready Open Liberty container images, an InstantOn application image can be built using the best practices for building optimized and secure application images. Such images will be ready to deploy into existing public clouds, such as AWS EKS and Azure AKS platforms. The rapid startup times that Liberty InstantOn provides make it an ideal platform for building your serverless applications.
