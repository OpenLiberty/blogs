...
To fine tune the langchain4j model or even try out another LLM, you simply update the values in the `langchain4j-examples/jakartaee-microprofile-example/src/main/resources/META-INF/microprofile-config.properties` file or provide them through the enviroment variables.
...
https://github.com/gkwan-ibm/langchain4j-examples/blob/jee-mp-example/jakartaee-microprofile-example/src/main/webapp/chatroom.js[`chatroom.js`]
[java]
----
...
The service recieves the user messeages, forward them to the LLM, and then boardcast the answers back to the client
...
and then broadcast the LLM responsed messages back to the client.
