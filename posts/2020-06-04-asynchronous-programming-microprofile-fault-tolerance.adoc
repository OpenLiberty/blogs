...
In this example, we'll use reactive methods in the JAX-RS client API for calling remote REST services asynchronously. These methods were introduced in JAX-RS 2.1. We can build up a request to fetch a String from a given URL with a GET request, where the return type is a `CompletionStage` of String:
