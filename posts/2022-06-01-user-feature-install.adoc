...
An Open Liberty user feature is a customized Open Liberty feature which consists of a definition file, or feature manifest, and a collection of link:https://www.ibm.com/docs/en/wasdtfe?topic=overview-osgi-bundles[OSGi bundles]. The OSGi bundles provide classes and services that correspond to providing a particular capability in the Open Liberty runtime environment.  
...
In this blog, we will use Eclipse with an Open Liberty link:https://start.openliberty.io[starter application] to create a Maven project for the user feature.
...
- Create a Bill of Materials (BOM) for your ESA file that will be consumed by Liberty Maven Plugin when preparing and installing the Open Liberty user feature.
...
The OSGi bundle is the unit of deployment for an application that has an independent lifecycle. You can create the OSGi bundle with any of your java classes, but in this example we will create a Java class that implements the `BundleActivator` interface to control the lifecycle of the OSGi bundle. When you start and stop the Open Liberty server, the start and stop methods of the `BundleActivator` interface are called.
...
+
In the previous step, we created the Java class that implements OSGi `BundleActivator`. Hence, we need to identify the bundle activator class to the OSGi framework by adding the `Bundle-Activator` header. Other headers are optional, but including the `Bundle-Name` and `Bundle-version` headers is a good practice.
+
[source, xml]
----
<plugin>
...
+
Open `${project.build.testOutputDirectory}/wlp/server.xml` file and add the user feature. The `usr` extension indicates that the feature will be installed to the `${project.build.testOutputDirectory}/wlp/usr` or `$WLP_USER_DIR` directory. It also tells the server where to find the user feature when the server starts.
+
[source, xml]
.server.xml
----
