...
The SmallRye GraphQL Client APIs are very similar to https://openliberty.io/guides/microprofile-rest-client.html[MicroProfile Rest Client] in that it uses an interface to invoke remote methods in a type-safe manner. For example, suppose we want a client that can invoke a query of all super heroes in a given location. We would create a query interface like this:
