---
layout: post
title: "Continuous integration and delivery (CI/CD) of Jakarta EE & MicroProfile applications to Kubernetes by using Jenkins"
categories: blog
...
---
= Continuous integration and delivery (CI/CD) of Jakarta EE & MicroProfile applications to Kubernetes by using Jenkins
...
[#Intro]
== DevOps, CI/CD, Jenkins & OpenLiberty (Jakarta EE & MicroProfile application)
...
== Installing and configuring Jenkins for CI/CD of Jakarta EE & MicroProfile applications
...
== Building the Liberty Java code with Jenkins

Before writing pipeline script to automate the deployment, you need to make a change in Dockerfile. The standard Dockerfile that you use in general is not enough to do the Liberty application deployment on containerized environment. A sample Openliberty compliant Dockerfile snipped in given, you can configure it as needed.
...
== Deploying Liberty applications to Kubernetes with Jenkins

For simplicity, I will use the command line (CLI) option to configure Jenkins to deploy a Jakarta EE & MicroProfile application to Kubernetes. You could use one of many tools though, such as Helm, Travis CI, Circle CI, etc.
...
When Jenkins has checked out the Jakarta EE & MicroProfile Java application code for the code build, all the Kubernetes configuration files are also downloaded to the Jenkins workspace so that Jenkins can run the IBM Cloud and Kubernetes commands to connect to the Kubernetes cluster and deploy the application.
...
== Conclusion
There are many ways in which you can configure your DevOps pipeline. This blog post is a quick introduction to how you can use Jenkins to set up a simple CI/CD pipeline to build and deploy your Jakarta EE & MicroProfile java applications.