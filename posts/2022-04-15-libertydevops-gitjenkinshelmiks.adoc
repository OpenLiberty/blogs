...
Build the container image for your application running on Liberty using a Dockerfile with the following content:
...
If you include the optional `featureUtility.properties` line, see link:/docs/latest/reference/command/featureUtility-commands.html#_define_custom_repositories[featureUtility commands] in the Open Liberty docs for how to create the properties file.

You can learn more about the Liberty container images available to you and best practices for creating a Dockerfile link:https://github.com/OpenLiberty/ci.docker/[here].
...
These tools are deployed in the Kubernetes cluster where the application is running and exposed by using route. You can access the gathered logs through the UI.
