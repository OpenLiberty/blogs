...
Setup the credential store server like vault and docker image scan server like aqua (if required) following the steps from their official product documentation.  (if you are not using any similar hosted services). Jenkins credentials can be used instead of vault.  
...
Next we should setup Jenkins, start with creating the required credentials in jenkins (like dockerhub creds, artifactory api creds, IKS api token, github personal access token etc), either directly on jenkins or integrate external credential store applications like vault to Jenkins and use the credentials from the same in Jenkins build jobs.  
