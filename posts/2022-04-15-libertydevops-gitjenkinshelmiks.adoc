...
* Maven repository and Docker image repository, like artifactory (optional)
     ...
     Create an IBM cloud image registry if required by following the steps in "https://cloud.ibm.com/docs/Registry?topic=Registry-getting-started" for pushing the docker images created for deployment. 
     You can use public docker hub registry as well for this. 
     Artifactory is another popular software in the market for this "https://www.jfrog.com/confluence/display/JFROG/Getting+Started+with+Artifactory+as+a+Docker+Registry"
