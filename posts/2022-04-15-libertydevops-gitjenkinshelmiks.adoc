...
Make sure that your Kubernetes configuration files are stored in the same git repository as your Jenkinsfile in a sub-directory called `deploy`. Also ensure that the Docker image name in the Kubernetes deployment configuration file are updated according to the name/tag in the Dockerfile ( manually or programmatically, if it needs to change at run time):
