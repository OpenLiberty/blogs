...
* A credential store application like harshicorp vault (optional)
     ...
     Your credentials (like dockerhub credentials, artifactory api credentials, IKS api token, github personal access token etc) can be securely saved within Jenkins itself on the page, http://localhost:8080/credentials/store/system/domain/_/newCredentials. 
