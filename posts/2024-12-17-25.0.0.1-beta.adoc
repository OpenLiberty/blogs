...
* <<aes, Support AES-256 password encoding>>
* <<instant, InstantOn support for Batch and Connectors Inbound Security>>
...
AES password encryption now uses an AES-256 bit key, and still supports existing AES password encryption with AES-128 bit keys. Previously, AES password encryption used only a 128-bit key encryption. The 256-bit key provides stronger encryption, making encrypted passwords more secure.

For example, you can run the `securityUtility encode` command with the `--aes` option. The following example also defines a custom encryption key with the `key` option. If you don't specify this option, a default key is used.

[source,bash]
----
wlp\bin>securityUtility encode --encoding=aes --key=Some_alternate_secret superAES256password
----

The response is a 256-bit AES encrypted password:
...
// // // // // // // //
[#instant]
== InstantOn support for Batch and Connectors Inbound Security
