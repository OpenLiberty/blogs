...
* Pipelines can be  `logs.app`, `logs.infra`, or `logs.audit`. For more information, see link:https://docs.openshift.com/container-platform/4.6/logging/cluster-logging-external.html#cluster-logging-collector-log-forward-fluentd_cluster-logging-external[Forwarding logs using the Fluentd forward protocol].
* If you are using a *secure* connection, specify the name of the secret that you created in step 2:
+
```
...
```
+
The following sample `log-forward-instance.yaml` file uses a *secure* connection:
+
```
