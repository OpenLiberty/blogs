...
+
The following sample `log-forward-instance.yaml` file uses secure connections and defines two outputs: `elasticsearch`, which routes to an internal Elasticsearch instance, and `fluentd-forward`, which routes to the instance of Fluentd that forwards the logs to Splunk. Each log type and its corresponding output reference is defined in the `pipelines` section:
+
```
