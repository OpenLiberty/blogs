...
*Note:* After you set the `managementState` to "Unmanaged" in the _ClusterLogging_ instance, any further changes to the _ClusterLogging_ or _ClusterLogForwarder_ instances are not automatically detected. If you need to fix or modify the configuration for either you will need to change the `managementState` field back to "Managed". However, once the `managementState` is changed to "Managed" the __ClusterLogging__ instance will revert the Fluentd's config map back to the original settings. You will need to complete the above steps again to enable Fluentd to parse JSON.
