...
+
The following sample `log-forward-instance.yaml` file defines a secure connections to a Fluentd server named `fluentd-server-secure` and an insecure connection to another Fluentd server named `fluentd-server-insecure`. Note the different protocols that are defined for both in the _url_ (i.e tls vs tcp) . Remove the output type that you do not wish to use in your file. Each log type and its corresponding output reference is defined in the `pipelines` section. The `default` output reference points to the internal OpenShift Container Platform Elasticsearch instance:
+
```
...
+
Select the "openshift" index in the `Available item(s)` box. This will move the index into the `Selected itm(s)` box.
+
...
Since Open Liberty application pods output logs in JSON format, it is recommended to set Fluentd to parse the JSON fields from the message body. To enable JSON parsing, first change the cluster logging instance's *managementState* field from *"Managed"* to *"Unmanaged"*:
...
Alternatively, you can modify this file on the web console. Ensure you are in the `openshift-logging` project namespace and go to `Administration > Custom Resource Definitions > ClusterLogging > Instances > Instance > YAML`

Next, we'll need to change the  *merge_json_log* attribute of the `kubernetes.var.log.containers.**` filter to *true* in the Fluentd's config map.
...
Alternatively, you can modify this file on the web console. Ensure you are in the `openshift-logging` project namespace and go to `Workloads > Config Maps > fluentd > YAML`

*Note:* After you set this value to *"Unmanaged"*, any further changes to the _ClusterLogging_ or _ClusterLogForwarder_ instances are not automatically detected. To automatically detect changes, you must change the *managementState* field back to *"Managed"*. By switching back to *"Managed"* the Cluster Logging instance will revert the Fluentd's config map back to the original settings.
