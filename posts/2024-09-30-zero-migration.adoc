...
If you use Maven or Gradle to build your application, update the `version` attribute of the `runtimeArtifact` property in the `pom.xml` file or the `libertyRuntime` property in `build.gradle` file, respectively. It's worth noting that if you don't specify the Liberty version in Maven or Gradle, it will default to the latest version available in the repository.
