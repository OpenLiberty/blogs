...
*Azure resource group:* Go to home page of link:https://portal.azure.com[Microsoft Azure] and click on "Create a resource" button. Search for "resource group" and create a resource group following the instruction. Resource group are a common identifier for all the related resources created in an Azure account. Resource groups are helpful for the resources that share the same lifecycle to easily deploy, update, and delete them as a group. +
*Azure container registry:* Go to home page of link:https://portal.azure.com[Microsoft Azure] and click on "Create a resource" button. Search for "container registry" and create a container registry following the instruction. Bind this container registry with the previously created 'resource group' selecting the same on the creation steps. +
