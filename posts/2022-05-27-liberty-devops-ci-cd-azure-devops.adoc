...
*Service connections:* We need to create 3 service connections for using in Azure DevOps pipeline
                      ...
                      3. **Snyk** Go to https://dev.azure.com/<your-org/ and select your Azure DevOps project. Click on the   
                         "Project settings" and click on "Service connections" under the section "Pipelines". Click on "New service connection" and search for "snyk", select "Snyk Authentication" and follow the further steps to create the service connection.     
...
*Azure build pipeline setup* Go to https://dev.azure.com/<your-organisation>/_git/<your-project> page and follow these steps to create a pipeline. +
              ...
              2. Select "Azure Repos Git" and then select your repository from the next page. +
              3. On "Configure your pipeline" page, select "Docker" (Build and push an image to Azure Container Registry) option. +
              4. Select your azure subscription and click on "continue" button. Sign in to microsoft account on popup. +
              5. Select container registry from the drop down, write the docker image name (which will be created and pushed to container registry by this build pipeline), make changes to default Dockerfile name (if any) and click on "Validate and configure" button. +
...
At this stage a pipeline yaml will be generated which you should configure according to your needs, make the follow changes +
              1. Modify the git trigger branch, if any +
                          trigger: +
                            - main +
              2. Under "variables" section in the yaml, change the image repository name to "liberty-app" ( or anything of your choice) and tag 
