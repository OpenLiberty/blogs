...
image::/img/blog/add-snyk-scan-to-stage.png[Add snyk scan to stage,width=90%,align="center"]
              ...
              * The last task required on build pipeline is to copy the deployment config file to azure staging repository for accessing the same on azure deployment pipeline (like the file 'azure-aks.yaml' in the below given example code). A sample Azure build pipeline code is given below. Save this code and run the pipeline 
