...
[start=2]
. In the Fluentd config map, change the `merge_json_log` attribute of the `kubernetes.var.log.containers.` filter to `true`. If you are running on OpenShift 4.3 to 4.5, this step is slightly different. Skip to the line that says *OpenShift 4.3 to 4.5 only*.
