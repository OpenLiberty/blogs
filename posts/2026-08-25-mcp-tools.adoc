...
The `<mcp>` element in `server.xml` lets you customise MCP endpoint behaviour per application.
...
Use the `moduleName` attribute on each `<mcp>` element to target a specific WAR within the EAR:
