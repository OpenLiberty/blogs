...
In this blog, I'll showcase how the new MicroProfile Language Server, along with existing tools for VS Code, makes it even easier to develop your Open Liberty applications.
...
* For Maven projects, add the https://github.com/OpenLiberty/ci.maven[Liberty Maven plug-in] to your `pom.xml` file:
+
[source,xml]
----
<plugin>
...
* For Gradle projects, add the https://github.com/OpenLiberty/ci.gradle[Liberty Gradle plug-in] to your `build.gradle` file:
+
[source,groovy]
----
