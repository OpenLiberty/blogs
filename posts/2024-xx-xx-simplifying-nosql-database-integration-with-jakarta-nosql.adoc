...
Many NoSQL databases types exists, but let's focus on the most famous types: Key-Value, Document, Column-family and Graph database types: key-value, document, wide-column and graph databases;
...
Jakarta NoSQL is a Jakarta EE specification designed to easily integrate Java and NoSQL databases. It uses common annotations and specific APIs for each of the NoSQL database types: key-value, wide-column, document databases.
...
Last but not least, Eclipse JNoSQL as a Jakarta Data implementation allow us to create repositories, offering a feature based on the Domain-Driven Development (DDD) Repository pattern, making helping developers to bring the code near to the business (domain centric) instead of the database semantics.
...
image::/img/blog/jnopo-architecture-before-persistence.gif[JNopo Sequence Diagram, align="center" width=60%]
