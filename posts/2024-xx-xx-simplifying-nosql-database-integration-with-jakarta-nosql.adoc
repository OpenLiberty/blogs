...
[source, java]
----
package org.jakartaee.sample.persistence;
...
@Repository
public interface Playoffs extends Data<GameMatch,String> {
...
=== Put all the components to play!
...
After some game matches, we can confirm the persistence of the game matches into the database by looking at Mongo Express instance running on http://localhost:8081 :
