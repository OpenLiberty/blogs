...
At this point, let's instantiate a new `GameMatch` entity instance, populate it from `GameOver` data and then store it into the database by using the `Playoffs` component, our repository offered by Jakarta Data specification implementation, Eclipse JNoSQL, which was injected with the `@Inject` CDI annotation:
...
Now, let's get started the OpenLiberty runtime to figure out if the game match results are being stored on the MongoDB:
...
OpenLiberty provides us an amazing tooling to help during the development of Jakarta EE / MicroProfile applications. When the dev mode is running, we can use some HTTP addresses to test and explore the capabilities offered by this amazing Jakarta EE/MicroProfile runtime:
...
* http://localhost:9080/openapi/ui/ : it's the Swagger UI interface that OpenLiberty offers out-of-the-box, facilitating the rest endpoint testing via browser.
...
. Now, in the "Query" session, run this command https://www.couchbase.com/products/n1ql/[N1QL] to create the primary index for the `GameMatch` collection:
+
[source, n1ql]
----
...
This challenge is an amazing opportunity to learn more about Jakarta NoSQL and Jakarta Data to simplify the NoSQL integration with Java applications.

What we learned:

* A practical example on how to use Sealed classes (since Java ??) and the benefits to use Pattern Matching (since Java ??) and Pattern Matching for Switch (since Java ??);
