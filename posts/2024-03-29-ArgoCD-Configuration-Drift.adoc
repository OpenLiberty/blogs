...
Now that you've installed ArgoCD and accessed its console, the next step is to set up a GitHub repository for ArgoCD to synchronize with. In our example, we created a deployment directory in the application code repo that we will point ArgoCD to for deployment.
...
4. Scroll down to the configuration section labelled **SOURCE**, as shown in the following image.
+
[.img_border_light]
...
Since we chose a manual sync policy, we have to sync the application manually for ArgoCD to deploy it. On the main page of the ArgoCD console, you should see a new tile that looks like this:
...
Now that the application is deployed and all the resources are created we need to make sure it is exposed externally.  This sample sets `spec.expose` to `true`, which automatically creates an external route on OpenShift.  If you are on Kubernetes, you need to either configure a load balancer or an Ingress controller for the application.

On OpenShift,  find the URL by running the following command:
...
Alternatively, if you are on Kubernetes with Ingress configured, ru the following command:
...
Let's say a developer changes the memory settings in the configuration file for a quick test.  To represent this, change the memory request found at `spec.resources.requests.memory` from 1024Mi to 2048Mi. To make this change, you can either use the console or run the following  Kubernetes CLI command:
...
"Out of Sync" means that some form of configuration drift occurred in the application.  We can refer back to the application in the ArgoCD UI to see exactly where it occurred.
