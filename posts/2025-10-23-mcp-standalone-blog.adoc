...
3. Find the version number of the `io.openliberty.mcp` jar, which is located within the `/dev/api/ibm/` folder inside the `wlp` folder. For example, if the `io.openliberty.mcp` jar is named `io.openliberty.mcp_1.0.106.jar`, then it means the version number is `1.0.106`.
