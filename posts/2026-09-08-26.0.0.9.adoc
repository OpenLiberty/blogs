...
The link:https://modelcontextprotocol.io/[Model Context Protocol (MCP)] is an open standard that enables AI applications to access real-time data and business logic from external sources. The MCP Server feature (`mcp-1.0`) lets application developers expose their Liberty application's tools and data for use in agentic AI workflows, with just a few annotations.

Before `mcp-1.0`, there was no standard way to expose your Liberty application's existing Java business logic to AI agents, meaning you would need to build and maintain custom integrations for each AI tool you wanted to connect to. With `mcp-1.0`, you annotate your existing CDI methods with `@Tool` and Liberty automatically handles the MCP protocol, tool discovery, session management, security, and observability. Your application becomes instantly usable by any MCP-compatible AI agent without changing your business logic.

=== What's new in 26.0.0.9

After several beta releases, `mcp-1.0` is now generally available. The most important change since the last beta (`26.0.0.9-beta`) is that the feature has been renamed from `mcpServer-1.0` to `mcp-1.0`, and the `server.xml` configuration element has been renamed from `<mcpServer>` to `<mcp>`. Update your `server.xml` as follows:
...
The following capabilities are also new in this GA release (not in 26.0.0.9-beta):
...
* *OAuth 2.0 Protected Resource Metadata (RFC 9728)* - When `protectedResourceMetaDataEnabled="true"` is configured on `openidConnectClient`, Liberty serves the protected resource metadata document at `/.well-known/oauth-protected-resource`, allowing MCP clients to automatically discover the correct authorization server following the MCP authorization flow.

For a full getting-started guide and examples, see the link:{url-prefix}/blog/2026/08/25/26.0.0.9-beta.html[Expose your Liberty business logic as AI tools blog post] and the link:https://openliberty.io/docs/latest/reference/feature/mcpServer-1.0.html[mcp-1.0 feature documentation].

[#protected_resource_metadata]
== Protected Resource Metadata for openidConnectClient

OAuth 2.0 Protected Resource Metadata (RFC 9720) provides metadata about a resource protected by OAuth 2.0, which can include a link to the authorization server which could grant access.

This release adds the ability for Open Liberty servers using the `openidConnectClient-1.0` feature to enable protected resource metadata. It can be enabled for any `openidConnectClient` configuration, but the primary use case is with the `mcp-1.0` feature to allow the MCP Authorization flow to be used. In this flow, the MCP client uses the protected resource metadata to discover the authorization server.

To enable protected resource metadata for an `openidConnectClient` configuration, simply add the `<protectedResourceMetadata>` configuration element:
...
When you run multiple Liberty instances that share the same JCache-based session store — for example, several pods on Kubernetes or OpenShift pointing at the same Hazelcast or Infinispan cluster — Liberty previously generated identical session cache names on every instance. This meant separate instances could unintentionally collide on the same cache, risking session data corruption in shared cache deployments.

This update adds a new `cacheNamePrefix` attribute to the `httpSessionCache` element, part of the existing `sessionCache-1.0` feature. It is aimed at operations teams and administrators running multi-instance Liberty deployments backed by a shared cache cluster, where distinct namespacing per instance is needed to avoid collisions.

Previously, session cache names were always generated as `com.ibm.ws.session.meta.*` and `com.ibm.ws.session.attr.*`, with no way to distinguish which Liberty instance created them. Now, each instance can be configured with its own prefix, so the same shared cluster can safely host session data from multiple Liberty servers without conflicts. If no prefix is set, behavior is unchanged and fully backward compatible.

To use it, add the attribute to your `httpSessionCache` configuration:
...
This results in distinct cache names for each instance — `pod1_com.ibm.ws.session.attr.default_host/myapp` and `pod2_com.ibm.ws.session.attr.default_host/myapp` — instead of both instances generating the same `com.ibm.ws.session.attr.default_host/myapp` name. This is especially useful for:
...
For more detail on configuring session caching, see the link:{url-prefix}/docs/latest/reference/config/httpSessionCache.html[httpSessionCache reference documentation].
