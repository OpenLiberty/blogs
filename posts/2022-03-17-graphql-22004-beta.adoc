...
seo-description: Open Liberty 22.0.0.4-beta offers GraphQL 2.0, incorporating Jakarta EE 9.1 dependencies, as well as Java 18 support with many new exciting features and changes. Also included in this release is distributed security caches, allowing multiple Liberty servers to share caches via a JCache provider.
...
Open Liberty 22.0.0.4-beta offers GraphQL 2.0, incorporating Jakarta EE 9.1 dependencies, as well as Java 18 support with many new exciting features and changes. Also included in this release is distributed security caches, allowing multiple Liberty servers to share caches via a JCache provider.
...
To give this a try today, link:https://jdk.java.net/18/[download the early access release of Java 18], <<run, install  22.0.0.4-beta>> version of Open Liberty, edit your Liberty server's link:https://openliberty.io/docs/latest/reference/config/server-configuration-overview.html#server-env[server.env file] to point `JAVA_HOME` to your Java 18 installation and start testing!
...
NOTE: When using the new distribute caching functionality in this beta release, the feature `distributedSecurityCache-1.0` needs to be included in the list of features in the `server.xml` file. This beta-only feature exposes the `JCache` APIs to the 3rd party `JCache` provider. When the distributed caching functionality is released as part of a GA, it will be exposed in a different manner then via the `distributedSecurityCache-1.0` feature, which will no longer be available.
...
To find out more, check out the link:https://openliberty.io/docs/latest/reference/config/authentication.html[authentication] and link:https://openliberty.io/docs/latest/reference/config/authCache.html[authCache] elements enabled by the link:https://www.openliberty.io/docs/latest/reference/feature/appSecurity.html[appSecurity] feature, and the link:https://www.openliberty.io/docs/latest/reference/feature/sessionCache.html#_examples[JCache Session Persistence examples].
