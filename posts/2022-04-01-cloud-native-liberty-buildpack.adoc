...
You can build your applicaiton without creating a Dockerfile!  
...
* Minimal Application Image - Images only contain what is necessary.
* Reproducability - Reproduces the same application image digest by re-running the build. 
...
== Now what? 
With your OCI image you can run your application locally:

[source]
docker run --rm -p 9080:9080 myapp 

or deploy your application to any Kubernetes-based platform such as link:https://www.redhat.com/en/technologies/cloud-computing/openshift[Red Hat OpenShift] using an link:https://github.com/OpenLiberty/open-liberty-operator[Open Liberty operator]
