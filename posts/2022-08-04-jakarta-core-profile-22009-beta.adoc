...
One of the new features is the addition of the link:https://jakarta.ee/specifications/expression-language/5.0/apidocs/jakarta.el/jakarta/el/methodreference[MethodReference] which "...provides information about the method to which a method expression resolves." This new class enables you to access any annotations that are present on a particular method by using the `getAnnotations()` method. The Expression Language 5.0 specification also includes a few deprecations and clarifications.
