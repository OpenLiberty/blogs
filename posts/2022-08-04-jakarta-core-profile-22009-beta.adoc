...
In previous versions of CDI, a non-empty `beans.xml` file that did not include a `version` attribute defaulted to `bean-discovery-mode="all"`. In CDI 4.0, it defaults to `bean-discovery-mode="annotated"`. It is recommended that all `beans.xml` files should be properly versioned.
...
The Liberty `<cdi12>` `server.xml` configuration element has been superseded by a new `<cdi>` element, which applies to CDI versions 1.2 and later.
