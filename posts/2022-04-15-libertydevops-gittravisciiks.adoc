---
layout: post
title: "Continuous integration and delivery (CI/CD) of Open Liberty applications to Kubernetes using Travis CI"
categories: blog
...
---
= Continuous integration and delivery (CI/CD) of Open Liberty applications to Kubernetes using Travis CI
...
In this blog i will give you a high level overview of using travis CI as a "code build tool" (with sample codes and required explanations), how you can connect with IBM cloud kubernetes service, docker image generation, scan the source code and opensource jar files, save the image to dockerhub repository and initiate the kubernetes deployment. Detailed explanation of technologies used in this blog, tool installation, complex devops pipeline design details of multi-environment application deployment are not in the scope of this blog. 

This blog explain the steps to deploy to IBM cloud kubernetes service for deployment (also added IBM cloud connectivity commands), but same steps explained will work for deployment on any other kubernetes service. 
...
=== Installing and configuring Travis CI and additional tools ===

Provision *Travis CI* from link:https://www.travis-ci.com/?_gl=1%2A1tiil9q%2A_ga%2AMTIwMjg2NTQ2NS4xNjUwNTUwODU5%2A_ga_XRYGSZFQ0P%2AMTY1MDkwOTQwMC40LjAuMTY1MDkwOTQwOC41Mg..[Travis CI] and integrate your GitHub repository with it ( when your personal profile or repository is integrated with travis CI, presense of a *.travis.yml* file in the repository will be enough for syncing that repository with travis CI). repository with  with all the recommended plugins link:https://www.jenkins.io/doc/book/installing/[install Jenkins], make sure that the following plugins are installed. You can install jenkins on any physical/virtual servers or it can be running as a container on Kubernetes itself.
