---
layout: post
title: "Continuous integration and delivery (CI/CD) of cloud-native Java applications using Travis CI"
categories: blog
author_picture: https://avatars3.githubusercontent.com/shamjithantholi
author_github: https://github.com/shamjithantholi
seo-title: Continuous integration and delivery (CI/CD) of cloud-native Java applications using Travis CI - OpenLiberty.io
seo-description: How to build a DevOps pipeline for a cloud-native Java application with Travis CI as the core controller component.
blog_description: How to build a DevOps pipeline for a cloud-native Java application with Travis CI as the core controller component.
...
DevOps is a culture or a process, which enables the integration of the software development (dev) and IT operations (ops). It can facilitate the setup of deployment pipeline and helps to improve the speed of software delivery. Automation of the entire build, test, and deployment of the software applications can be achieved through DevOps process. Tons of efficient applications and process methods are available in the market that can be readily integrated to the DevOps pipeline, which can help to achieve the wanted results.

CI/CD are acronym’s for continuous integration, and continuous delivery or deployment used in DevOps. Continuous integration is a process where developers frequently merge code changes into a source code repository, which automatically start code build, security scan, and test execution and deployment. Automation of this process enables organizations to release on a more frequent basis without compromising on quality. Various tools are available to automate the CI/CD process. This blog post discusses how you can use Travis CI to automate the CI/CD process for building, testing, and deploying your Jakarta EE & MicroProfile application with Open Liberty

Travis CI is a continuous integration service use to build and test the code. Travis CI works with various Git-based services (for example: GitHub, Bitbucket).

link:https://www.redhat.com/en/technologies/cloud-computing/openshift[OpenShift] is a containerization platform from RedHat. Container orchestration in Red Hat OpenShift is managed by Kubernetes and its build around linux containers.
...
The following diagram shows a simple architecture for a pipeline that builds and deploys a Jakarta EE & MicroProfile application with Open Liberty into a single Kubernetes environment. When a code change is detected by Travis CI, Travis builds the code, runs security and static analysis scans, and runs unit tests against the built code. Travis CI then generates a Docker image of the application with its Open Liberty server and saves the image to a Docker repository, Travis then trigger the deployment using CLI commands and deploy the Docker image to OpenShift.

image::/img/blog/liberty-devops-generic-architecture-new.png[Liberty devops generic architecture diagram ,width=70%,align="center"]

Basic understanding of Git, Docker, and containerization concepts are a prerequisite for this blog.

== Setting up Travis CI and OpenShift for CI/CD of Jakarta EE & MicroProfile applications with Open Liberty ==
...
Provision *Travis CI* from link:https://www.travis-ci.com/?_gl=1%2A1tiil9q%2A_ga%2AMTIwMjg2NTQ2NS4xNjUwNTUwODU5%2A_ga_XRYGSZFQ0P%2AMTY1MDkwOTQwMC40LjAuMTY1MDkwOTQwOC41Mg..[Travis CI] and integrate your GitHub repository with it (when your personal profile or repository is integrated with travis CI, presense of a *.travis.yml* file in the repository will be enough for syncing that repository with travis CI). 
     ...
     --> Click on "Sync Account" button to sync new repositories  
...
Provision the OpenShift cluster on IBM cloud (https://cloud.ibm.com), generate OpenShift token for CLI connectivity, verify the basic k8s cluster login commands (Note: For working with the steps explained in this blog, you can use any other kubernetes service of your choice ).     
...
Create a project/namespace. Create Red Hat OpenShift secrets to checkout the code from GitHub to Red Hat OpenShift (upload the public to GitHub ) and another one for accessing DockerHub repository from OpenShift
...
This command creates the build, deployment, and service on Red Hat OpenShift automatically. Verify that if the deployment is done successfully by login to OpenShift UI. Create a route on OpenShift and verify whether the endpoints are reachable.

When deployment is successful to local image stream, edit the OpenShift BuildConfig and DeploymentConfig to use the DockerHub repository for storing and pulling images.
...
As explained earlier, *.travis.yml* file must be available on the base path of the GitHub repository. An example view of GitHub repository base path is given
...
Travis CI and OpenShift provide wide variety of build and deployment options, given is a workable sample travis CI code and Dockerfile that is good enough to automate the code build
...
* Run the docker image scan by using aqua trivy
