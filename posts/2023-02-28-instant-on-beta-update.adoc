...
For the Open Liberty 23.0.0.2-beta, we removed the need for the `net_admin` capability for both the checkpoint and restore of a process. Also, the `sys_ptrace` capability is no longer necessary to restore the process when running the application with Liberty InstantOn. This change is an improvement; the `net_admin` and `sys_ptrace` capabilities grant a broad set of capabilities that might be unacceptable when deploying an application to the cloud. For 23.0.0.2-beta, we did add a requirement for the `setpcap` capability to enable the process checkpoint during a container build.
