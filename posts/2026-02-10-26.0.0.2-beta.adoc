...
The `mcpServer-1.0` feature does not currently implement all of the MCP specification’s link:https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization#authorization-server-metadata-discovery[authorization-server-metadata-discovery requirements]. However, you can use any of Liberty's link:https://openliberty.io/docs/latest/authentication.html[authentication and authorization mechanisms] to authenticate users and assign roles. The `mcpServer-1.0` feature then can ensure that users can access only the tools that are permitted for their assigned roles.
...
If the `server.xml` excludes a role found in a `@RolesAllowed` annotation, no user receives that role or its permissions. 
