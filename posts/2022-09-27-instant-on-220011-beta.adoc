...
With that understanding, we can now build the container image using the `podman build` command. Run the following command to download or update to the latest Open Liberty container image:
[source]
----
...
podman run \
  ...
  --privileged \ <1>
  --env WLP_CHECKPOINT=applications \ <2>
  rest-intro-application
----
<1> The `--privileged` option is required to perform the `criu` checkpoint in-container.
<2> The `WLP_CHECKPOINT` environment variable is used to specify the checkpoint phase. For the rest-intro example the `applications` checkpoint phase will provide the fastest restore time.
...
----
podman run \
  --cap-add=CHECKPOINT_RESTORE \ <1>
  --cap-add=NET_ADMIN \
  --cap-add=SYS_PTRACE \
  --security-opt seccomp=unconfined \ <2>
  --security-opt systempaths=unconfined \
  --security-opt apparmor=unconfined \
  -p 9080:9080 \
  rest-intro-instanton
----
<1> Use `--cap-add` options to grant the container the three Linux capabilities that `criu` requires.
<2> The `--security-opt` options are necessary to grant `crui` access to the required system calls and access to `/proc/sys/kernel/ns_last_pid` from the host. 
...
podman run \
  ...
  rest-intro-instanton
----
<1> The `--security-opt seccomp=` option refers to a file called <<sys-calls-json,`criuRequiredSysCalls.json`>>. This file specifies the system calls required by `criu`.
