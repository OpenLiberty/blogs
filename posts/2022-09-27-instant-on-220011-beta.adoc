...
You further simplify the checkpoint process by reducing the need for the `--security-opt` options that use `unconfined`. By default, `podman` does not grant access to all the system calls that `criu` needs (defaults specified in the file `/usr/share/containers/seccomp.json`).  First, you need an additional configuration file that grants all the required system calls that `criu` needs to the container. Second, the host `/proc/sys/kernel/ns_last_pid` needs to be mounted. You can do both these steps with the following command:
