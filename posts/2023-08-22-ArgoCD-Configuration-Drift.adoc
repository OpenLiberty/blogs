...
<mst>We assume that your CI pipleline has a way to build contianer image for your application. For this example, we can either build the application or use the pre-built cotnainer image used in the deployment files.

<mst> Are the below commands in the README? Can we remove them?
...
Upload the image to an image repository that OpenShift can access (Docker Hub or Quay, for example)
...
<mst> /replace above with this 
Now that we have the Git configuratio repository ready for deployment, it’s time to configure ArgoCD to deploy the application.
...
<mst> I think we need to add this section?
Let us add something like what we do in demo? update something like (CPU or memory) directly from the console and how ArgoCD detects it and gives your opportunity to resolve it.
