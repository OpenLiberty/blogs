...
On DEV environment, all the common config files and DEV specific config files can be stored on Git on a separate repository (not along with the source code repository, a separate repository can be created for configs - in this way, we can stop exposing the source code to non-DEV team).  
...
on PROD environment, deployment pattern is same as on QA environment, PROD team can setup the deployment config in the Git repository (get the latest docker image details from development team who is responsible to create it) and execute the same from build tool (like Jenkins) and initiate the redeployment of the image.
