...
Various configuration property information are injected from the microprofile-config.properties in order to connect to the database. 
...
See AtlasMongoDB.java. The role of the AtlasMongoDB.java class is to provide relevent functions for RAG such as creating search indexes, or retrieving similar content.
...
Similarily, the annotations such as APIResponses indicate the response code with the descriptions.
...
If the database is empty, the search index is created using the methods as described in AtlasMongoDB. The code goes through the resources/knowledge_base files using the provided MD_FILES.
Each line of the text file is extracted where the first line is stored as the summary, and the rest as the content. + 
The summary, content, and embedding vector are created and added in the same document. + 
...
The next `PUT` request updates the existing content and summary given the object id at `/api/embedding/{id}`. 
...
The appropriate status code is returned. 
...
1. ModelBuilder: +
    ...
    The embedding model is the model that is used to embed the text segements (ie. convert text into vectors).+
...
Environment variables are used to set these properties.
...
The getChatModel function in `ModelBuilder` sets the ChatModel by using a langchain4j APIs.
In order to configure the model, the model name, the api key, temperature, maxtokens and other variables are set.
...
The getEmbeddingModel() constructs the EmbeddingModel using respective embedding models names from the provider.
For example, in this application, if the ollama provider was being used, then nomic-embed-text model name is chosen. 
...
The modelBuilder instance is used to create an Assistant and the Assistant has access to max number of messages sessional chat history.
