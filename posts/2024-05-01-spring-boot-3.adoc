...
----
+
4. You can also configure the application in the Liberty `server.xml` file by setting a Spring Boot property as an application argument. The following example sets a logger level for the application.
+
[source,xml]
----
...
+
  Hello from Spring Boot in Open Liberty!
+
9. When you finish testing the application, stop the server by pressing `Ctrl`+`C`.
