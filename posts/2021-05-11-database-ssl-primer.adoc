...
Most of the configuration in this post is for the database JDBC driver, which ypou can easily configure on Open Liberty by enabling one of the `JDBC-4.x` features and configuring the the `dataSource` element in your `server.xml` file.
...
As a result, every JDBC driver provider that supports TLS connections has their own implementations and each JDBC driver has to be configured differently.
...
- `oracle.net.authentication_services`: Setting TCPS tells the driver to expect the database to authenticate the connection to the database via the TLS handshake.  That is, if the TLS handshake succeeds, no other authentication is needed to access the database.  This property requires additional setup on the database.  Notice  that in this example, neither datasource has an `containerAuthDataRef` attribute. The container authorization is not needed because the `oracle.net.authentication_services=TCPS` property indicates that the TLS handshake is the only authorization that is required.
...
The driver must compare the certificate sent by the database with a trusted certificate.
To establish trust, configure one of the following options:

- Option 1: Set the `trustServerCertificate` attribute to `true`.  This tells the driver to always trust any certificate that is sent by the database. This option is helpful when debugging your configuration, but should not be used in a production environment.
...
Each section helps point out the debug that can show a potential issues with your configuration and suggestions on how to fix it.
...
If you get an exception similar to the following example, it likely means that the database (server) sent a certificate in its `ServerHello` message that the driver was not able to find a trusted certificate to verify against:
...
If you have configured your driver to send a certificate back to your database to verify, as in the previous Oracle example, then that verification could also fail.
...
In other cases, the driver sends a certificate, but database cannot not verify it. This issue is difficult to debug because the issue is on the database side. The driver might throw one of many different exception to indicate that it could not finish the handshake.

The following sample exception shows an error message when the driver tries to connect to a SQLServer database that the database cannot verify:
