...
seo-title: Enabling Jakarta EE 9 for development and test by using the Eclipse Transforme
...
In this post, we describe how to use the Eclipse Transformer to update your applications and test artifacts so that they can run by using Jakarta EE 9. We also describe the steps to update Open Liberty server configurations to run Jakarta EE 9 applications.
...
Although this post focuses on updating from Jakarta EE 8 to Jakarta EE 9, the same information holds true for updating from Java EE7 to Jakarta EE 9.

== Test transformation strategy

To enable Jakarta EE 9 tests in Open Liberty, you can annotate test classes and methods to indicate whether they are to be run as Jakarta EE 9 tests. The Open Liberty test framework then transforms and reruns the annotated tests as Jakarta EE 9 tests.

Two types of updates are required to transform test artifacts. First, package references must be updated, changing old `javax` package references into new `jakarta` package references. Package references can be in several locations, including test applications, test code, and server configurations. Open Liberty updates package references by using the Eclipse Transformer.
...
////
This part is confusing. Does the test framework update the feature names? In the "Updating features in server configurations" it says "Currently, feature names must be updated manually." But this section seems to indicate that the "test framework" will do it, or that "the transformer can be used to update features", if you customize it, which is not explained. We should be more clear up front:

- What can the transformer do? What does it not do?
- what does the "test framework" do? How does a user know how to "annotate test classes and methods to indicate whether they are to be run as Jakarta EE 9 tests". We don't explain this but it sounds like the post will tell you how.

Most of this info is in the post (I think) but it needs to be more clearly stated so the reader knows what to expect.
////

Alternatively, the transformer can be used to update features in the server configuration if you create rules data with the substitution values that are needed for this update. In this case, you must either create code that is similar to the test framework code or update server configurations manually.
////
^^ Does this apply to any other info in this post? If not we should at least give a link for more info. As is, it might be understood to say the next section will tell you how to write rules data
////
...
////
Is this an example of something the dev would have to do manually? Or does the transformer do it? It's not clear
////

Open Liberty server configuration elements that contain "javax" package references must be transformed to run Jakarta EE 9 enabled applications. For example, the `destinationType` attribute of the `jmsActivationSpec` element contains a `javax` package reference:
...
Currently, feature names must be updated manually. The transformer cannot be used because it is missing rules data that specify feature name updates.
