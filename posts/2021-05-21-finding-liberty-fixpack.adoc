...
In some cases, you can follow a chain of revert PRs and re-introduce issues until you end up with a commit in a PR that was successfully merged. Look for a re-introduce message, switch to the issue that it identifies, and repeat the procedure that was described previously.
