...
The transformer can be run using custom rules by specifying additional options on the command line.  The additional options specify local rules files that the transformer is to be used instead of the default rules files.
...
Add three command line options to the transformer command line invocation to pull in the more up-to-date rules data:
...
Main Eclipse Transformer project page: link:https://projects.eclipse.org/projects/technology.transformer[Eclipse Transformer].

A direct link to the transformer GIT repository: link:https://github.com/OpenLiberty/open-liberty/tree/integration/dev/wlp-jakartaee-transform[dev/wlp-jakartaee-transform].

Links to transformer downloads: link:https://projects.eclipse.org/projects/technology.transformer/downloads[Downloads],

For current snapshot: link:https://oss.sonatype.org/content/repositories/snapshots/org/eclipse/transformer/org.eclipse.transformer.cli/[Snapshots].
...
Example gradle tasks for using the transformer: link:https://github.com/OpenLiberty/open-liberty/blob/integration/dev/wlp-gradle/subprojects/fat.gradle[dev/wlp-gradle/subprojects/fat.gradle]
...
A typical transformer invocation: link:https://github.com/OpenLiberty/open-liberty/blob/integration/dev/fattest.simplicity/src/componenttest/rules/repeater/JakartaEE9Action.java#L204[JakartaEE9Action.transformApp].
