...
You can use Spring Boot 3.x capabilities in your Liberty applications with the new Spring Boot Support 3.0 feature. You can also now authenticate your OpenID Connect clients using the more secure Private Key method instead of client secrets, and, if you use LTPA or JWT cookies, you can now use different cookies for different applications, according to their context root.
