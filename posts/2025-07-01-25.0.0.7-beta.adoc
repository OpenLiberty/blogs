...
Then deploy rest application and then visit `/openapi` on liberty server to view the OpenAPI documentation, or visit `/openapi/ui` for a user interface that lets to browse through it. To enhance the documentation, use link:https://download.eclipse.org/microprofile/microprofile-open-api-4.1/apidocs/[annotations] in your code to add things like textual descriptions to each method. The annotations are available as a maven dependency:
...
MicroProfile Telemetry 2.1(`mpTelemetry-2.1`) provides developers with the latest Open Telemetry technology as the feature now consumes OpenTelemetry-1.48.0 which has been updated from 1.39.0. API's are now stable.
