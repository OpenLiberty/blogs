...
Some features - such as `@Schema`, `DefaultValueConverter`, `ToolManager`, and `ToolResponseEncoder` - are provided by the `io.openliberty.mcp` jar that ships with Liberty. To make these available on the build path, add a `system`-scoped dependency inside your `pom.xml` file that points to the jar inside your Liberty installation:
...
Find the `io.openliberty.mcp_*.jar` in `<wlp>/dev/api/ibm/` and record the version suffix (e.g. `1.0.106`). Then add:
