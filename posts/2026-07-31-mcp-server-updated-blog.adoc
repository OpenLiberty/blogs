---
layout: post
title: "Expose your Liberty business logic as AI tools - a complete guide to MCP server"
categories: blog
...
The `mcpServer-1.0` feature uses the https://github.com/mcp-java/java-mcp-annotations[`mcp-java` shared API] (`org.mcpjava:mcp-server-api`), which is available on Maven Central.
...
To connect an AI agent to your MCP server, provide the MCP endpoint URL from the `CWMCM0008I` log message in the client's configuration. For example, to add your MCP server to https://www.ibm.com/products/watsonx-ai[IBM Bob], open `~/.bob/settings/mcp.json` and add an entry to the `mcpServers` object:
...
Tool methods can return `String`, POJO objects (serialised to JSON), or typed content objects from the `org.mcpjava.server.content` package: https://www.javadoc.io/doc/org.mcpjava/mcp-server-api/latest/org/mcpjava/server/content/TextContent.html[`TextContent`], https://www.javadoc.io/doc/org.mcpjava/mcp-server-api/latest/org/mcpjava/server/content/ImageContent.html[`ImageContent`], https://www.javadoc.io/doc/org.mcpjava/mcp-server-api/latest/org/mcpjava/server/content/AudioContent.html[`AudioContent`], `EmbeddedResource`, and `ResourceLink`.
...
NOTE: The special parameters https://www.javadoc.io/doc/org.mcpjava/mcp-server-api/latest/org/mcpjava/server/Cancellation.html[`Cancellation`] and https://www.javadoc.io/doc/org.mcpjava/mcp-server-api/latest/org/mcpjava/server/progress/Progress.html[`Progress`] are also supported on async tool methods.
...
In stateless mode, each request is handled independently with no per-client state between calls.
...
The `asyncTimeout` can be updated dynamically — changing `server.xml` while the server is running applies the new value immediately without a restart.
...
== Connecting desktop AI clients with OAuth

When connecting desktop MCP clients (such as Claude Desktop or VS Code with GitHub Copilot) to a Liberty MCP server over the internet, you typically need an authenticated connection. Liberty supports https://datatracker.ietf.org/doc/rfc9728/[OAuth 2.0 Protected Resource Metadata (RFC 9728)], which enables automatic OAuth login flows in interactive clients. When Liberty is configured as an OAuth or OIDC resource server alongside `mcpServer-1.0`, MCP clients can automatically discover which authorisation server to use to obtain an access token.

To enable this, add the OIDC resource server features alongside `mcpServer-1.0`:
...
NOTE: This feature is currently in beta. A detailed step-by-step configuration guide for desktop MCP clients will be published as a separate blog post.
