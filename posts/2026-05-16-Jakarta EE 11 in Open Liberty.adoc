...
Jakarta Contexts and Dependency Injection (CDI) 4.1 brings important architectural improvements and new APIs to help framework developers build on CDI. CDI allows objects to be bound to lifecycle contexts, injected into application code, be subject to interceptors and decorators, and interact in a loosely coupled fashion via events.
...
public class LoggedInterceptor {
    ...
    public Object logInvocation(InvocationContext context) throws Exception {
        // NEW in CDI 4.1: Retrieve interceptor bindings
        Set<Annotation> bindings = context.getInterceptorBindings();
        
        // Find the @Logged annotation and extract its value
        String logName = bindings.stream()
            .filter(a -> a.annotationType().equals(Logged.class))
            .map(a -> ((Logged) a).value())
            .findFirst()
            .orElse("unknown");
...
Note, you can also qualifier custom HTTP Authentication Mechanisms (as you could prior to Jakarta Security 4.0) and inject the custom HAM into your custom HAM handler.
