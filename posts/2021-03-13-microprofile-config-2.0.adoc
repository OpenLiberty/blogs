...
- <<Config-Profile, Config Profile>>- A special property, called `mp.config.profile`, can be used to determine which set of config property values are used.
- <<ConfigValue, ConfigValue>>- An API class which holds a variety of useful information about a specified config property.
...
- <<Property-Expression, Property Expression>>- A way of embedding config property values inside of other config property values using the `${}` syntax.
