...
1. You'll also need to enable the link:https://openliberty.io/docs/latest/reference/feature/feature-overview.html[feature] in Open Liberty. 
+
You’ll need to enable `mpConfig-2.0` (MicroProfile Config 2.0’s feature name in Open Liberty) in your server.xml in addition to `cdi-2.0` which enables all the cool injection functionally:
+
.server.xml
[source,xml]
----
...
----
+
Alternativelly, you could enable the `microProfile-4.0` link:https://openliberty.io/docs/latest/reference/feature/microProfile-4.0.html[convenience feature], which enables `mpConfig-2.0` and `cdi-2.0` and all the other awesome MicroProfile features.
...
With Config Profiles, your microservice reacts appropriately based on the project stage without changing **any** code and without updating a bunch of config values manually.
...
[source,java]
----
serverDetails.host;  // Retrieves the value, as a String, for the config property named server.host. Returns: example.org
serverDetails.port;  // Retrieves the value, as an int, for the config property named server.port. Returns: 9080
----
...
You can also do some funky expressions such as defining default values, composed expressions, and multiple expressions. link:https://download.eclipse.org/microprofile/microprofile-config-2.0/microprofile-config-spec-2.0.html#property-expressions[The spec] covers these really well.
...
* From MicroProfile Config 2.0, these "empty" values are no longer valid. Retrieving the values "natively" will now throw a `NoSuchElementException`. E.g. for the values defined in the <<example-ConfigSource2, example ConfigSource v2>>:
+
[source,java]
----
@Inject
...
config.getValue("empty.property", String.class); // Will throw a `NoSuchElementException`
config.getValue("empty.array.property", String[].class); // Will throw a `NoSuchElementException`
----
+
