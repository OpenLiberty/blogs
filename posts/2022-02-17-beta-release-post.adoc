...
It's only been a couple of short months since we announced Jakarta EE 9.1 support in Open Liberty 21.0.0.12, and we're already looking forward as we collaborate on and begin to implement Jakarta EE 10 features.  Open Liberty 22.0.0.3-beta offers our first Jakarta EE 10 beta feature, Concurrency 3.0.  Concurrency 3.0 introduces a variety of new enhancements including resource definition annotations that put applications in control of defining the concurrency constraints and thread context types that the application requires, context-aware completion stages that are managed by the container, and more.
