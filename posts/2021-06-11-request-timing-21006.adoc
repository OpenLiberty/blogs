...
---
= Request Timing Metrics plus a number of significant bug fixesin Open Liberty 21.0.0.6
...
Open Liberty 21.0.0.6 brings improvements to the MicroProfile Metrics feature, allowing for information gathered via the Request Timing feature to be displayed on the metrics endpoint.
...
View the list of fixed bugs in link:https://github.com/OpenLiberty/open-liberty/issues?q=label%3Arelease%3A21.0.0.6+label%3A%22release+bug%22[21.0.0.6].
// end::intro[]
...
image::/img/blog/new_Metrics_Grafana-21006.png[requestCountThings,width=50%,align="center"]
...
The issue occurred intermittently at server startup and arised when transaction recovery was attempted before data source initialization has been completed. In this situation the server would report `InternalLogExceptions` and failed to start.
...
+
When calling a protected `URL` including some query params, the server uses OIDC. The `URL` in the `WASReqURL` cookie was incorrectly encoded instead of the URL query params to be decoded upon successful redirection. To fix this issue instead of encoding the query string in the original request `URL`, we are leaving it as-is. That should ensure we ultimately use the `real` value of the query string. During the creation of the `WASReqURLOidc` cookie, the cookie value is encoded anyway to mitigate against malformed and potentially malicious cookie values. The `OidcRedirectServlet` code is also updated to use slightly different code to read the cookie so that the respective decoding is done.
...
+
When the liberty server was started the first balid JWT Token in JwtAI results 
