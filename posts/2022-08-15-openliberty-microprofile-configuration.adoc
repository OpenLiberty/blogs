...
MicroProfile and Open Liberty have standardized the use of these parameter files with additional properties like ordianls. Different type of files where the constants and run time variables can be hosted are 
            ...
            5. CustomConfig properties (anything which implement org.eclipse.microprofile.config.spi.ConfigSource)
...
There are multiple ways with which you can inject data into containers. This blog will discuss about multiple types of MicroProfile and Open Liberty configuration files and the ways to inject dynamic values into the corresponding variable in the container. 
...
PODS can define environment variables directly in the pod definition yaml configuration and application code can use it. Any change in environment variables will need a pod restart for the change to take effect in the code.
