...
To give this a try today, link:https://jdk.java.net/20/[download the early access release of Java 20], install the <<run,23.0.0.3-beta>> version of Open Liberty, edit your Liberty server's link:{url-prefix}/docs/latest/reference/config/server-configuration-overview.html#server-env[server.env file] to point `JAVA_HOME` to your Java 20 installation and start testing!
