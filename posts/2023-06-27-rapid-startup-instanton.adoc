...
[.img_border_light]
image::/img/blog/checkpoint4.png[diagram of the checkpoint and restore process,width=70%,align="center"]
...
An InstantOn checkpoint is created by starting the Open Liberty runtime during the application container image build step. During this startup, the runtime processes the configuration, loads all the enabled features, and starts processing the configured application. Depending on the needs of your application, you can choose one of two specific phases during Open Liberty startup at which to checkpoint the process. You must configure the Dockerfile to specify you chosen phase (as we show later).
