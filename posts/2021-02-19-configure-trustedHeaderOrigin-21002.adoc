...
+
This issue originated from "link:https://github.com/OpenLiberty/open-liberty/issues/14417[<include location> tag does not consider variables when installing server.xml]". When the include tag entry was `/${shared.config.dir}/server.xml"` and the environment variable was `WLP_USER_DIR=/test/`, the expected path was `test/config/server.xml' but the user received an error stating that "it couldn't find file at /user/test/wlp/usr/shared/config/server.xml".
This bug causes include tags to not be parsed correctly because the environment variables are not taken into account found link:https://www.ibm.com/support/knowledgecenter/en/SSEQTP_liberty/com.ibm.websphere.wlp.doc/ae/twlp_admin_customvars.html[here] 
