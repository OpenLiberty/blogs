...
* link:https://github.com/OpenLiberty/open-liberty/issues/15433[System WABs may come online with the web container after server reports started]
+
When a WAB is included in a Liberty feature (also called a system WAB) it is possible that the server started message and the TCP ports will be opened and listening before the WAB has been fully provisioned with the web container. This is different behaviour to web applications WARs where the server started and TCP ports are delayed until all the applications have started or a timeout has occurred on server start.
...
+
This issue oringated from "link:https://github.com/OpenLiberty/open-liberty/issues/14417[<include location> tag does not consider variables when installing server.xml]". When the entry was `/${shared.config.dir}/server.xml”` and the environment variable was `WLP_USER_DIR=/test/` in the `test/config/server.xml` file the user received an error stating that "it couldn't find file at /user/test/wlp/usr/shared/config/server.xml" where it should have been looking for `test/config/server.xml`.

This bug causes include tags to not be prased and replaces the environment variables found link:https://www.ibm.com/support/knowledgecenter/en/SSEQTP_liberty/com.ibm.websphere.wlp.doc/ae/twlp_admin_customvars.html[here] 

This was fixed by tapping into existing server configuration parsing libraries to find the user directory instead of a custom made method.
