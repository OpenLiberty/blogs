...
+
When compression is configured in the `server.xml` on an `httpendpoint` and `http/2` is used, the `http/2` max frame size may be exceeded, leading to an `http/2` `FRAME_SIZE_ERROR` in the server log.
+
