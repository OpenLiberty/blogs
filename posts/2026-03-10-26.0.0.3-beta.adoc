...
seo-description: This beta release of Open Liberty updates the `mcpServer-1.0` feature by introducing encoders, and providing tools access to the request ID.
blog_description: This beta release of Open Liberty updates the `mcpServer-1.0` feature by introducing encoders, and providing tools access to the request ID.
...
This beta release of Open Liberty updates the `mcpServer-1.0` feature by introducing encoders, and providing tools access to the request ID.
...
When a client calls a tool, the object returned by the tool method is converted to a `ToolResponse` which usually contains one or more `Content` objects. `ToolResponse` maps directly to the result in the response returned when calling a tool.
