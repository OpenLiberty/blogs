...
查看 link:https://github.com/OpenLiberty/open-liberty/issues?q=label%3Arelease%3A26004+label%3A%22release+bug%22[26.0.0.4] 中修复的错误列表。
...
以前，只能在 `server.xml` 文件中为每个配置配置一个签名算法（例如，`RS256`）。如果传入的 JWT 使用不同的算法签名，验证将失败。此更新允许使用 JWT 头部中的签名算法进行验证。它提供了在单个配置中使用不同签名算法的灵活性。
...
当使用 `FROM_HEADER` 与非对称算法和信任库设置时，相应公钥的别名必须以其相应的算法为前缀（例如，`RS256_keyalias`）以进行自动选择。别名名称的其余部分无关紧要，只要它以签名算法字符串开头即可。在验证期间，服务器会在信任库中搜索以 JWT 头部中指定的算法开头的别名。如果未找到算法前缀的别名，客户端将回退到使用 `trustedAlias` 属性（对于 `jwtConsumer`）或 `trustAliasName` 属性（对于 `openidConnectClient`、`oidcLogin` 和 `mpJwt`）指定的别名（如果已配置）。如果信任库中存在多个具有签名算法前缀的别名，Liberty 将使用找到的第一个。
...
[#java_26]
=== 对 Java 26 的支持
Java 26 是最近的 Java 版本，引入了比早期版本更多的新功能和增强功能，值得查看。此版本不是长期支持 (LTS) 版本。
...
Java 26 中的新变更 JEP 500（"准备使 Final 真正意味着 Final"）开始通过限制使用深度反射时的变异来强制执行 final 字段的真正不可变性。
在 Java 26 中，此类变异仍然有效，但默认情况下会触发运行时警告，为开发人员准备更严格的执行。
