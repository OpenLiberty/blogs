...
在将应用克隆到本地之前，请先安装 JDK 17，并确保已正确设置 `JAVA_HOME` 环境变量。你可以选择使用 https://developer.ibm.com/languages/java/semeru-runtimes/downloads[IBM Semeru Runtime] 作为您的 Java 运行时。该运行时基于 Eclipse OpenJ9 等项目的深度技术投入，在多种硬件和软件平台上都具备良好的性能优势。更多关于 IBM Semeru Runtime 的信息，请参阅文章 link:https://openliberty.io/blog/2022/08/19/open-liberty-semeru-performance.html[Open Liberty 和 Semeru Runtimes，重要的云原生性能]。
...
如果你尚未安装 Git，可以参考 link:https://git-scm.com/book/en/v2/Getting-Started-Installing-Git[Git 安装指南]进行安装。安装完成后，运行以下命令将 `langchain4j-examples` GitHub 仓库克隆到本地：
...
如果你对 LangChain4j API 的其他使用方式感兴趣，可以参考此文件中提供的 REST API 示例：link:{example-location}/src/main/java/dev/langchain4j/example/rest/ModelResource.java[src/main/java/dev/langchain4j/example/rest/ModelResource.java]
