...
seo-description: 通过使用 LangChain4j API，在运行于 Open Liberty 上的 Jakarta EE 和 MicroProfile 应用程序中体验 AI 技术。
blog_description: "通过使用 LangChain4j API，在运行于 Open Liberty 上的 Jakarta EE 和 MicroProfile 应用程序中体验 AI 技术。"
...
语言模型是基于概率的自然语言模型，它们能够生成一系列词语组合的概率。而https://www.ibm.com/cn-zh/think/topics/large-language-models[大型语言模型（LLM）]是指规模庞大的语言模型，通常以其参数数量巨大而被分类为“大型”。这些模型通过自监督和半监督学习技术，在海量数据上进行训练，参数量可能达到数十亿级别。训练后的模型具备生成自然语言及其他类型内容的能力，能够执行广泛的任务。

你可以观看这个link:https://www.youtube.com/watch?v=5sLYAQS9sWQ[介绍视频]，了解 LLM 的基本概念、工作原理以及在商业中的应用场景。
...
虽然 LangChain 主要提供 Python 和 JavaScript/TypeScript 版本，但社区项目如 https://github.com/langchain4j/langchain4j[LangChain4j] 也提供了在 Java 应用中使用 LangChain 的选项。通过 LangChain4j API，开发者可以将 LLM 集成到 Java 应用中，并连接到不同的 AI 平台，如 OpenAI 和 Hugging Face。
...
在将应用克隆到本地之前，请先安装 JDK 17，并确保已正确设置 `JAVA_HOME` 环境变量。你可以选择使用 https://developer.ibm.com/languages/java/semeru-runtimes/downloads[IBM Semeru Runtime] 作为您的 Java 运行时。该运行时基于 Eclipse OpenJ9 等项目的深度技术投入，在多种硬件和软件平台上都具备良好的性能优势。更多关于 IBM Semeru Runtime 的信息，请参阅 https://openliberty.io/blog/2022/08/19/open-liberty-semeru-performance.html[Open Liberty and Semeru Runtimes, cloud-native performance that matters]。

该应用程序使用 Hugging Face。您需要获取 Hugging Face API 密钥：

* 注册并登录 https://huggingface.co
* 前往 link:https://huggingface.co/settings/tokens[访问令牌页面]
* 创建一个具有 `read` 权限的访问令牌
...
[#tryout]
== 尝试 jakartaee-microprofile-example 应用程序

要了解如何将 LangChain4j 应用到您自己的 Jakarta EE 和 MicroProfile 应用程序中，请亲自查看这个示例项目。

[#pre-reqs]
=== 前提条件
在将应用程序克隆到您的机器之前，请安装 JDK 17，并确保设置了 `JAVA_HOME` 环境变量。您可以使用 https://developer.ibm.com/languages/java/semeru-runtimes/downloads[IBM Semeru Runtime] 作为您选择的 Java 运行时。这个运行时通过在 Eclipse OpenJ9 等项目中的深度技术投资提供性能优势，并且可在各种硬件和软件平台上使用。要了解有关 IBM Semeru Runtime 的更多信息，请参阅 https://openliberty.io/blog/2022/08/19/open-liberty-semeru-performance.html[Open Liberty 和 Semeru Runtimes，重要的云原生性能]。
...
如果你尚未安装 Git，可以参考 https://git-scm.com/book/en/v2/Getting-Started-Installing-Git[Git 安装指南]进行安装。安装完成后，运行以下命令将 `langchain4j-examples` GitHub 仓库克隆到本地：
...
要运行该应用，请进入 `jakartaee-microprofile-example` 目录：
...
要启动该应用程序，请使用项目中提供的 Maven Wrapper，在 Open Liberty https://openliberty.io/docs/latest/development-mode.html[dev mode] 下运行：
...
为确认应用是否成功启动，你可以在命令行中按下 `enter/return` 键运行测试。如果测试通过，你将看到类似如下的输出：
...
应用启动后，你可以通过任意浏览器访问 http://localhost:9080/ 并开始试验。
...
你可以输入任意文本与 AI 代理进行对话。以下是一些推荐的消息示例：
...
请参阅文件：{example-location}/src/main/java/dev/langchain4j/example/chat/ChatAgent.java[src/main/java/dev/langchain4j/example/chat/ChatAgent.java]
...
要微调 LangChain4j 模型或尝试其他 LLM，只需更新配置文件 {example-location}/src/main/resources/META-INF/microprofile-config.properties[langchain4j-examples/jakartaee-microprofile-example/src/main/resources/META-INF/microprofile-config.properties] 中的值或通过环境变量提供它们。
...
请参阅文件：{example-location}/src/main/webapp/chatroom.js[src/main/webapp/chatroom.js] file
...
请参阅文件：{example-location}/src/main/java/dev/langchain4j/example/chat/ChatService.java[src/main/java/dev/langchain4j/example/chat/ChatService.java]
...
如果你对 LangChain4j API 的其他使用方式感兴趣，可以参考此文件中提供的 REST API 示例：{example-location}/src/main/java/dev/langchain4j/example/rest/ModelResource.java[src/main/java/dev/langchain4j/example/rest/ModelResource.java]
