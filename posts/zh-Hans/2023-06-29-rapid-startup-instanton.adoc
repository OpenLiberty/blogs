...
官方 link:/docs/latest/container-images.html[IBM Container Registry (ICR) 的Open Liberty镜像] 包含了InstantOn检查应用程序进程所需的所有先决条件。对于本例， `getting-started` 入门应用程序容器镜像使用来自ICR的 `icr.io/appcafe/open-liberty:full-java11-openj9-ubi` 映像作为父镜像。目前，InstantOn只支持基于Java 11和Java 17的Open Liberty UBI镜像。
