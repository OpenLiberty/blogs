...
link:{url-about}[Open Liberty] 26.0.0.7 では次の変更があります:

* <<container_image, LibertyのコンテナイメージをUBI 10 Minimalに更新>>
...
// // // // // // // //
ランタイムに新機能を追加し、<<guides, ガイド>>も更新しました。
...
link:{url-prefix}/blog/?search=release&search!=beta[以前の Open Liberty GA リリースのブログ記事]もご確認ください。
...
link:{url-prefix}/docs/latest/container-images.html[コンテナー・イメージ]を使用している場合は以下です。
...
もしくは、link:{url-prefix}/start/[ダウンロード・ページ]をご覧ください。
...
[#container_image]
== LibertyコンテナイメージをUBI 10 Minimalに更新

Open Liberty 26.0.0.7以降、UBI MinimalをベースとするすべてのLibertyコンテナイメージ（-ubi-minimalタグが付いたイメージ）は、これまでのUBI 9 Minimalではなく、Red Hat Universal Base Image（UBI）10 Minimalをベースとして構築されるようになりました。

コンテナイメージのタグ名自体は変更されないため、イメージの参照先を更新する必要はありません。

今回の更新により、Libertyコンテナイメージのセキュリティが向上します。これは、OSパッケージに対するセキュリティ修正がUBI 10 Minimalではより早く提供されることが多く、その結果、作成されるコンテナイメージで報告されるCVEの数を減らせるためです。

26.0.0.7より前の既存のLibertyリリースでは、引き続きUBI 9 Minimalが使用されます。たとえば、26.0.0.6はUBI 9 Minimalのままであり、サポート対象のライフサイクル期間中は、OSおよびJavaのセキュリティアップデートを引き続き受け取ります。

ユーザーは、ビルド環境および実行環境が、基盤となるUBI 10 / RHEL 10と互換性があることを確認してください。

詳細については、link:https://www.ibm.com/support/pages/announcement-upcoming-changes-container-images-ibm-websphere-liberty-and-open-liberty[アナウンスメント]およびlink:https://www.ibm.com/docs/en/was-liberty/base?topic=images-liberty-container[ドキュメント]を参照してください。
...
詳細については、ログアウトされた SSO Cookie の追跡に関するlink:https://openliberty.io/docs/latest/track-loggedout-sso.html[資料]をご確認ください。
...
mpHealth-4.0 の機能は、Open Liberty に MicroProfile Health 4.0 のサポートを追加します。この機能には、/health REST エンドポイントの代替として、ファイル・ベースのヘルス・チェック・メカニズムが含まれています。ファイル・ベースのヘルス・チェック・メカニズムが有効な場合、サーバー構成の属性を使用して `/health/started`、`/health/live`、および `/health/ready` REST エンドポイントを無効にできます。
...
* link:https://github.com/OpenLiberty/open-liberty/issues/35179[PathUtils.normalize() が "${" で始まるセグメントの後にある ".." の折りたたみ処理をスキップする]
* link:https://github.com/OpenLiberty/open-liberty/issues/35121[サーバー作成時に、「java.lang.System 内の制限されたメソッドが呼び出されました」という警告が表示される]
