...
Open Liberty は、`server.xml` ファイルに保存されるパスワードの Advanced Encryption Standard (AES) 暗号化をサポートしています。この AES 暗号化は、現在 AES-256 ビット・キーを使用しています。AES 復号化については、Open Liberty は AES-128 と AES-256 の両方をサポートしています。以前は、Open Liberty の AES パスワード暗号化と復号化は 128 ビット・キーのみを使用していました。256 ビット・キーはより強力な暗号化を提供し、暗号化されたパスワードをより安全にします。

AES-256 暗号化でパスワードを暗号化するには、`wlp/bin` ディレクトリから `securityUtility encode` コマンドを `--encoding=aes` オプションを付けて実行します。
...
このパスワードを `server.xml` ファイルで使用できます。例えば、キーストア定義で以下のように使用します。
...
- **Deprecated** とマークされているものを除くすべてのガイドが、MicroProfile 7 と link:{url-prefix}/docs/latest/reference/feature/versionless-features.html[バージョンレス機能] を使用するように更新されました。
