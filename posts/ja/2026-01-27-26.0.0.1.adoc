...
ログ抑制は、`throttleType` ロギング属性を使用して、メッセージまたは messageID に基づいてメッセージを抑制するように構成できます。抑制が開始される前に許可されるメッセージの数は、`throttleMaxMessagesPerWindow` 属性を使用して構成できます。

ログ抑制は、`throttleMaxMessagesPerWindow` を `0` に設定することで無効にできます。
...
`throttleType` が `messageID` に設定されている場合、抑制は messageID の発生回数のみに基づいて適用されます。この例では、`TEST0111I` が抑制に使用されます。

`throttleType` が `message` に設定されている場合、抑制はメッセージ全体に適用されます。メッセージ内容のバリエーションは個別に追跡されます。この例では、`TEST0111I: Hello World!` が抑制に使用されます。
