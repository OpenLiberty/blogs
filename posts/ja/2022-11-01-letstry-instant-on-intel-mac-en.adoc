---
layout: post
title: Let's try Open Liberty InstantOn on Intel Mac
# Do NOT change the categories section
categories: blog
author_picture: https://avatars3.githubusercontent.com/takakiyo
author_github: https://github.com/takakiyo
seo-title: Let's try Open Liberty InstantOn on Intel Mac - OpenLiberty.io
seo-description: Let's try Open Liberty InstantOn on Intel Mac
blog_description: "Let's try Open Liberty InstantOn on Intel Mac"
...
Liberty InstantOn was announced on September 29, 2022. It is a technology that launches a full-featured Java EE application server in hundreds of milliseconds.
...
Open Liberty/WebSphere Liberty with an OpenJ9 VM incorporates CRUI technology to get a Checkpoint when the application finishes initializing. By committing the container in this state and resuming it from there, it can be started in a lightning fast time.

== Let's try InstantOn on Intel Mac

The InstantOn steps in the blog article link:https://openliberty.io/blog/2022/09/29/instant-on-beta.html[Liberty InstantOn startup for cloud native Java applications] are  based on the Red Hat Enterprise Linux. The same experience can be achieved on an Intel Mac with macOS Monterey (version 12.x). This article shares the steps to experience the exciting technology on Mac. 

The InstantOn sample environment is published as a container image from IBM Cloud Registry. As the container execution environment, we can use podman introduced by Homebrew. On Mac, we needed to tweak the steps a little bit to see the InstantOn in action. 

=== Preparing the Podman

Since InstantOn uses the CAP_CHECKPOINT_RESTORE function of the Linux kernel, the InstantOn feature does not work properly if podman is started in rootless mode, Also, once the virtual environment (podman machine) has been started in rootless mode, it will fail even if it is run in rootful mode. The solution is to create a new dedicated virtual machine.
...
Following the version of the podman used for this article. 
...
Since the container was started without `--rm`` this time, the container environment after execution remains as it is. In this environment, files that record process information with checkpoint are saved.
...
The following command commits and register it as a new image.
