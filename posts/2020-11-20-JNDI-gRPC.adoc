...
The error handling of this invalid configuration was improved by giving the user an error message of:
[source, text]
----
...
+
This issue can cause variables defined in configuration files included by server.xml to not be recognized. We added support for using variables in include elements sometime earlier this year, and to do that we have to temporarily add new variables into the registry we use to resolve values. But in this specific scenario it affected the computation we do later to determine if any variables have changed. We would end up removing variables from the permanent variable registry. For more information take a look at our link:https://openliberty.io/docs/20.0.0.11/reference/config/server-configuration-overview.html[Server Configuration Overview].
