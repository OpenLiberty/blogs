...
+
MYFACES-4594 was opened because `org.apache.myfaces.LOG_WEB_CONTEXT_PARAMS` defaults to `dev-only`, which is defined here: https://myfaces.apache.org/#/core40
+
...
+
When you enable the FIPS feature with the IBM Semeru JDK and OpenID connect client enabled, this FFDC exception is emitted:
+
[source, xml]
----
