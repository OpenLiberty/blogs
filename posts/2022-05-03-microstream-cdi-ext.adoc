...
blog_description: "The MicroStream CDI extension can be used to integrate the framework into a MicroProfile application that runs on Open Liberty. It brings you the capabilities of storing Java Object Graphs natively and secure, relieved of heavy-weight DBMS dependencies."
...
Microservices have become a buzzword when we talk about creating a scalable application. As with any software architecture decision, it has a trade-off and several challenges. One of those challenges is the distributed nature of the environment.  Data access is another challenge and since many solutions make use of data that is stored in an entirely different format as the Java Objects, a database for example with tables and columns, accessing data can be slow due to the mapping that is required.
...
Let’s first have a quick tour of MicroStream.  MicroStream allows you to write a Java Object graph very efficiently and fast to data storage. This can be the disk, blob storage of a database, or any other medium. Since your data is stored in memory, it provides microsecond query time, low-latency data access, and gigantic data throughput since there is no mapping as the data are just Java instances. There is no mapping and conversion to a database system needed.

Whenever the StorageManger is started, the previously stored content is loaded into memory, and from that moment on, all data access is just accessing Java instances in memory which is very fast and efficient. The storage with MicroStream is also highly secure, it stores only the instance variables and a class identification. It cannot reconstruct some arbitrary class instances as you do with Java Serialisation.

When you have updated or changed the data that needs to be stored, perform a _store_ operation and data is placed on the external resource again so that it can be read the next time when you start the application.
