...
Depending on your specific use case, you may need to apply additional constraints by link:https://www.ibm.com/support/pages/fips-140-3-cryptography-ibm-semeru-runtimes#:~:text=Creating%20extensions%20of%20profiles[creating an extension of the profile]. For instance, if your application uses PKCS12 as the TLS certificate keystore and truststore, you’ll need to define a custom profile that includes support for PKCS12.

Refer to this link:https://www.ibm.com/support/pages/fips-140-3-cryptography-ibm-semeru-runtimes#:~:text=PKCS12%20and%20JKS%20Example[example profile] for guidance on how to extend the strict profile to accommodate PKCS12-based configurations.

To support functionality beyond the default `OpenJCEPlusFIPS.FIPS140-3` profile, you must update your custom profile with additional constraints. These constraints should be carefully reviewed to ensure compliance with the FIPS 140-3 specification for your specific use case. 
...
In the example above, a separate file is used to define the custom profile. This is the recommended approach for link:https://www.ibm.com/support/pages/fips-140-3-cryptography-ibm-semeru-runtimes#:~:text=Creating%20extensions%20of%20profiles[creating extensions of profiles] in IBM Semeru Runtimes.
