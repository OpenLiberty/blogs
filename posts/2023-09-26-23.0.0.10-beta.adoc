...
This beta previews the abundance of new capabilities that the Open Liberty team are currently working on, including MicroProfile 6.1 (Metrics, Telemetry, and OpenAPI) and Java 21. It also introduces the ability to give your Spring Boot applications faster startup times with little or no extra effort by using Liberty InstantOn; if you have any Spring apps to hand, give it a try. There are a couple of new updates too that make it easier to manage security configurations in containerized environments, and a preview of Jakarta Data beta 3.
...
* <<mpmetrics, MicroProfile Metrics 5.1>>
