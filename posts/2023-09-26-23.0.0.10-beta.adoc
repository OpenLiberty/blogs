...
+
Specifying validation keys this way can be combined with enabling monitor directory to also use validation keys that are not specified in server.xml configuration at the same time. For example:
