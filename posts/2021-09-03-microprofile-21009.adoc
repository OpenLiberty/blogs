...
+
In the `mpOpenAPI` feature, an incorrect validation message could be incorrectly displayed when using the `multipleOf` attribute of the `@org.eclipse.microprofile.openapi.annotations.media.Schema` annotation. With `multipleOf = 1`, the server shows:
+
[source, role="no_copy"]
----
