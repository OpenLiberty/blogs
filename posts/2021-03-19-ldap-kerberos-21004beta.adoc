...
LDAP Bind operations are used to authenticate clients (and the users or applications behind them) to the directory server. This establishes an authorization identity that will be used for subsequent operations processed on that connection, and to specifies the LDAP protocol version that the client will use. Prior to this update, LDAP would allow binding either anonymously or with a `bindDN` user and a `bindPassword` (simple authentication), this update adds a new option to bind to LDAP: GSSAPI/Kerberos. Kerberos is an authentication mechanism that allows a client and service to mutually authenticate via a Key Distribution Center (KDC). In Open Liberty 21.0.0.4-beta using both a Kerberos credential cache (ccache) or a Kerberos keytab file is supported.
...
Below is an example of congiuring an `LdapRegistry` using a ticket cache and custom Kerberos config file:
...
----
dependencies {
    libertyRuntime group: 'io.openliberty.beta', name: 'openliberty-runtime', version: '[20.0.0.4-beta,)'
}
----
...
----
dependencies {
    libertyRuntime group: 'io.openliberty.beta', name: 'openliberty-jakartaee9', version: '[20.0.0.4-beta,)'
}
----
