...
MicroProfile Metrics 2.0 introduces multi-dimensional metrics through the use of metric tags. This is an evolution from Metrics 1.x which only allowed a metric name to be used with only one set of tags. Now multiple related metrics can be easily queried and monitored by name through a monitoring tool. This allows for the related metrics to be easily queried and monitored by name through a monitoring tool.
...
Lastly, Microprofile Metrics 2.0 uses the new multi-dimensional metric capability by auto-tagging metrics with an `pass:[_]app` tag with the value retrieved from the `pass:[_]mp.metrics.appName_ mpConfig` value, if defined. This is especially useful if multiple apps are running because it tags each individual application's metrics with the name of the application.
